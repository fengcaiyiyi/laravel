        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <form action="{{ URL::route('feimo/editCategory',[$id]) }}" class="form-horizontal" method="post" id="formdata">
...
<script>
    $(function () {
        $('.btn-add').click(function () {
            ...
            $("#formdata").ajaxSubmit({
                type: 'post',
                url: "{{ URL::route('feimo/editCategory',[$id]) }}",
                success: function (data) {
                    if(data==1){
                        notify('error', 'top center', '分类已存在');
                    }else if (data == 'ok') {
                        notify('success', 'top center', '修改成功');
                        setTimeout(function () {
    ...
    $("#select").find('option[value="{{$info->parentid}}"]').attr("selected", true);
</script>
